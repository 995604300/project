...
class Rbac
{
    ...
    public function createPermission(array $data = [])
    {
        ...
        $res = $res = Db::name($this->permissionTable)->where('name', $data['name'])->find();
        if (!empty($res)) {
            throw new Exception('权限名重复');
        }
    ...
    public function editPermission(array $data = [], $id = null)
    {
        ...
        $res = $res = Db::name($this->permissionTable)->where('name', $data['name'])->where('id','neq', $id)->find();
        if (!empty($res)) {
            throw new Exception('权限名重复');
        }
     ...
     * 传入的条件当为主键时直接按照主键查询，如果为条件语句必须符合tp5的where条件写法
     */
    public function getRoleList($size ,$condition, $conditionOr)
    {
        $dbObj = Db::name($this->roleTable);
        if (is_numeric($condition)) {
            return $dbObj->where($dbObj->getPk(), $condition)->find();
        }
        $subQuery = Db::name($this->userRoleTable)
                      ->alias('b')
                      ->field('count(b.id)')
                      ->where('a.id = b.role_id')
                      ->buildSql();

        return $dbObj->alias('a')
            ->field("a.*,$subQuery AS role_number,c.nick_name as add_people_name")
            ->join('rly_user c','a.add_people = c.id','left')
            ->where($condition)
            ->whereOr($conditionOr)
            ->paginate($size)
            ->toArray();
    ...
    {
        $res = Db::name($this->rolePermissionTable)
          ->alias('a')
          ->join("$this->permissionTable p",'a.permission_id = p.id')
          ->where('role_id', $id)
          ->select();
        return $res;
    }
    ...
    public function assignUserRole($userId, array $role = [])
    {
        ...
        foreach ($role as $v)
        {
            if (is_array($userId)) {
                foreach ($userId as $value) {
                    $userRole [] = ['user_id' => $value, 'role_id' => $v];
                }
            } else {
        ...
        return Db::name($this->userRoleTable)->insertAll($userRole,true);
    }
    ...
    public function clearUserRole($userId)
    {
        ...
        return Db::name($this->userRoleTable)->where('user_id',$userId)->delete();
    }
     ...
     * 清除为角色分配的权限
     */
    public function clearRolePermission($roleId)
    {
        if (empty($roleId)) {
            throw new Exception('参数错误');
        }

        return Db::name($this->rolePermissionTable)->where('role_id',$roleId)->delete();
    }
    ...
    public function cachePermission($id)
    {
        ...
        cache($id."_permission", $newPermission);
        return true;
    }
    ...
    {
        $permissionList = cache($id."_permission");
