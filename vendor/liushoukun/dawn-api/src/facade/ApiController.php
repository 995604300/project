    ...
    private function _execAuth()
    {
        ...
        if (self::_getConfig('api_debug')) {
            ...
            try {
                ...
                if ($auth !== true) throw new UnauthorizedException('未经授权');

            } catch (UnauthorizedException $e) {
                $end = getMicrotime();
                //授权认证失败
                throw  new HttpResponseException($this->sendError(($end - $start), 401, $e->getMessage()));
            } catch (Exception $e) {
                $end = getMicrotime();
                throw  new HttpResponseException($this->sendError(($end - $start), 500,'server error'));
            }
        }
    }
