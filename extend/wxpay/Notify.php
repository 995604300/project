...
class Notify extends \WxPayNotify
{
    ...
    public function NotifyProcess($data, &$msg)
    {
        ...
        if(!array_key_exists("transaction_id", $data)){
            ...
            db('order')->where('out_trade_no', $data['out_trade_no'])->update(['pay_state'=>2, 'out_trade_no'=>$data['out_trade_no'], 'transaction_id'=>$data['transaction_id'],'pay_dateTime'=>$pay_time]);
            return false;
        }
    ...
    public function processOrder($order, $data)
    {
        ...
        // 例如:
        $pay_time = date("Y-m-d H:i:s",time());
        $result = db('order')->where('id', $order['id'])->update(['pay_state'=>1, 'out_trade_no'=>$data['out_trade_no'], 'transaction_id'=>$data['transaction_id'],'pay_dateTime'=>$pay_time]);
        return $result;
    }
    ...
    public function getOrder($data)
    {
        // 可根据商户订单号进行查询
        // 例如:
        $order = db('order')->where('order_no', $data['out_trade_no'])->find();
        return $order;
    }
    ...
    public function checkOrderStatus($order)
    {
        ...
        // 例如:
        return $order['pay_state'] == 1 ? true : false;
    }
}
